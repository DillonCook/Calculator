  ...
  return (
    <section className="rounded-2xl border border-white/10 bg-panel p-5 shadow-soft">
      ...
      <div className="space-y-3">
        ...
        <Section title="Variable Expense Matrix (per strategy)">
          <div className="space-y-2">
            {value.variableExpenses.map((expense, index) => (
              <div key={expense.key} className="rounded-lg border border-white/10 bg-white/[0.03] p-2">
                <div className="grid gap-2 sm:grid-cols-[1.2fr_160px_1fr] sm:items-center">
                  <p className="text-sm font-medium">{expense.label}</p>
                  <Input
                    label="Amount / mo"
                    type="number"
                    value={expense.monthlyAmount}
                    onChange={(v) => updateVariableExpense(index, { monthlyAmount: Number(v) })}
                  />
        ...
        <Section title="Long-Term Rental">
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            <PercentInput label="Maintenance reserve %" value={value.longTerm.maintenancePercent} onChange={(v) => update('longTerm', 'maintenancePercent', v)} />
            <PercentInput label="CapEx reserve %" value={value.longTerm.capexPercent} onChange={(v) => update('longTerm', 'capexPercent', v)} />
