        ...
        {rows.map((row) => {
          ...
          return (
            <div key={row.key} className="rounded-xl border border-white/10 bg-white/5 p-3">
              ...
              <div className="mt-2 grid grid-cols-2 gap-1 text-xs text-muted sm:grid-cols-4">
                <span>CoC {percent.format(output.cashOnCashReturn)}</span>
                <span>ROI {percent.format(output.roi)}</span>
                <span>Cap {percent.format(output.capRate)}</span>
                <span className="sm:text-right">DSCR {output.dscr > 0 ? output.dscr.toFixed(2) : 'n/a'}</span>
              </div>
            </div>
          );
        })}
