          ...
          <svg viewBox="0 0 100 40" className="absolute inset-x-0 bottom-0 h-[44%] w-full" preserveAspectRatio="none" aria-hidden="true">
            <defs>
              <linearGradient id={gradientId} x1="0" y1="0" x2="1" y2="0">
                <stop offset="0%" stopColor="#7BA7FF" stopOpacity="0.8" />
                <stop offset="50%" stopColor="#4F8DFD" stopOpacity="0.95" />
                <stop offset="100%" stopColor="#6E7E9C" stopOpacity="0.78" />
              </linearGradient>
              <linearGradient id={areaGradientId} x1="0" y1="0" x2="0" y2="1">
                <stop offset="0%" stopColor="#4F8DFD" stopOpacity="0.35" />
                <stop offset="100%" stopColor="#6E7E9C" stopOpacity="0.05" />
              </linearGradient>
            ...
            {linePath ? <path d={linePath} fill="none" stroke={`url(#${gradientId})`} strokeWidth="1.8" filter={`url(#${filterId})`} /> : null}
            {chartPoints.map((point, index) => (
              <circle key={`${label}-point-${index}`} cx={point.x} cy={point.y} r="0.95" fill="#4F8DFD" opacity="0.55" />
            ))}
          </svg>
        </div>
      ) : null}
