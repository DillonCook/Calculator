...
export function KpiCard({ label, value, helper, winner, tone = 'default', secondaryLabel, secondaryValue, definitions }: KpiCardProps) {
  ...
  return (
    <div className="rounded-2xl card-surface p-2.5 shadow-soft sm:p-4">
      <div className="flex items-center gap-2">
        <p className="text-[11px] uppercase tracking-wide text-muted sm:text-xs">{label}</p>
        {definitions?.length ? (
      ...
      <p
        ref={primaryValueRef}
        className={`mt-1 text-xl font-semibold leading-tight sm:text-3xl md:text-4xl ${tone === 'success' ? 'text-emerald-300' : 'text-white'}`}
      ...
      {secondaryLabel && secondaryValue ? (
        <div className="mt-2 rounded-lg border border-white/10 bg-white/[0.03] px-2 py-1.5 sm:px-2.5 sm:py-2">
          ...
          <p ref={secondaryValueRef} className="text-sm font-semibold text-white sm:text-lg" data-testid={`kpi-${slugify(secondaryLabel)}`}>
            {secondaryValue}
          </p>
        </div>
      ) : null}
