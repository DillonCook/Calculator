...
const contentDate = (route: string) => {
  ...
  if (!date) throw new Error(`Missing explicit content date for ${route}`);
  return date;
};
...
const citations = {
  ...
  noi: { label: 'Fannie Mae Multifamily Guide — Net Cash Flow', href: 'https://mfguide.fanniemae.com/node/2756', note: 'Effective gross income and operating expenses are inputs to property net cash flow; exact lender conventions vary.' },
  capRate: { label: 'Freddie Mac — Capitalization Rate Guidance', href: 'https://mf.freddiemac.com/docs/capitalization_rate_guidance.pdf', note: 'Cap-rate analysis depends on supported NOI, market evidence, and professional judgment; a calculator output is not an appraisal.' },
...
<section class="hero">
  ...
  <div class="hero-copy">
    ...
    <p class="hero-lede">DealCooker is a free real estate underwriting calculator for rental, Airbnb, room-by-room, BRRRR, flip, and small commercial deals—built to show what the assumptions produce and which supported levers may improve the model.</p>
  ...
  <div class="hero-product" aria-label="DealCooker product preview">
    <div class="product-window">
      ...
      <div class="window-grid">
        ...
        <div class="work-card"><span>Make the deal work</span><h2>Compare the supported levers behind a weak result.</h2><div><b>Workout levers</b><strong>Price + down payment</strong></div><a href="${appHref('product-preview-workout', 'homepage_cta')}">Open the workout</a></div>
      </div>
...
<section class="make-it-work section-shell" id="how-it-works">
  ...
  <div class="work-layout">
    <div class="work-story"><p>When a modeled deal misses DealCooker’s built-in cash-flow or DSCR thresholds, the workout can test a lower purchase price and, when applicable, a larger down payment. Cash-deal and flip workflows use their own strategy-specific target logic.</p><ul><li><span>01</span>Choose the strategy and enter the deal.</li><li><span>02</span>See whether the current inputs clear the model’s thresholds.</li><li><span>03</span>Compare any supported price or equity scenario with the original.</li></ul><a class="text-link" href="${appHref('make-it-work', 'homepage_cta')}">Open the workout <span>↗</span></a></div>
    <div class="work-visual"><div class="before-after"><div><small>CURRENT ANALYSIS</small><strong>Your inputs</strong><span>Modeled without a promised outcome</span><i>Original scenario</i></div><div class="arrow">→</div><div><small>WORKOUT ANALYSIS</small><strong>Supported levers</strong><span>Lower price or more equity, when applicable</span><i class="positive">Compared side by side</i></div></div><div class="recommendation"><span>Make the deal work</span><strong>Calculate the adjustment from the actual property inputs.</strong><a href="${appHref('workout-preview', 'homepage_cta')}">Open the workout</a></div></div>
  </div>
</section>
...
const strategyBody = (page: Page) => {
  ...
  const dateModified = contentDate(`/${page.slug}/`);
...
<section class="strategy-work section-shell"><div><p class="eyebrow">When the deal misses</p><h2>Don’t stop at “no.”<br><em>Test the supported levers.</em></h2><p>DealCooker can test a lower purchase price and, when applicable, a larger down payment against the active strategy’s workout logic. It does not optimize interest rates, points, loan products, or every financing term.</p></div><div class="recommendation large"><span>MAKE THE DEAL WORK</span><strong>Compare supported price and equity scenarios.</strong><a class="button button-primary" href="${appHref(`${page.source}-workout`, 'seo_strategy_page', appStrategy)}">Try it free ↗</a></div></section>
...
<section class="methodology-content section-shell">
  ...
  <div class="definition-grid">
    <article><h2>Net operating income (NOI)</h2><p>Modeled property income after vacancy and operating costs, before debt service. Depending on the strategy, DealCooker’s modeled NOI may include reserves and fixed costs such as PMI, so it may differ from lender or accounting NOI.</p></article>
    ...
    <article><h2>Cap rate</h2><p>Annual modeled NOI divided by the basis used for that strategy. Most acquisition views use purchase price; BRRRR uses ARV. Because modeled NOI can include financing-dependent fixed costs, treat this as a DealCooker metric—not a universal unlevered cap rate or appraisal.</p></article>
...
write('methodology/index.html', shell({
  ...
  schema: [
    { '@context': 'https://schema.org', '@type': 'WebPage', name: 'DealCooker Methodology', url: `${SITE_ORIGIN}/methodology/`, dateModified: contentDate('/methodology/'), author: organizationSchema, creator: { '@type': 'Person', name: AUTHOR.name, url: AUTHOR.url }, about: softwareSchema },
...
for (const page of pages) {
  write(`${page.slug}/index.html`, shell({
    ...
    schema: [
      ...
      { '@context': 'https://schema.org', '@type': 'WebPage', name: page.h1, url: `${SITE_ORIGIN}/${page.slug}/`, description: page.description, dateModified: contentDate(`/${page.slug}/`), author: organizationSchema, creator: { '@type': 'Person', name: AUTHOR.name, url: AUTHOR.url }, isPartOf: { '@type': 'WebSite', name: 'DealCooker', url: `${SITE_ORIGIN}/` }, about: { '@type': 'SoftwareApplication', name: 'DealCooker', url: APP_ORIGIN } },
...
write('sitemap.xml', `<?xml version="1.0" encoding="UTF-8"?>\n<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">\n${sitemapRoutes.map((route) => `  <url><loc>${SITE_ORIGIN}${route}</loc><lastmod>${contentDate(route)}</lastmod></url>`).join('\n')}\n</urlset>\n`);
